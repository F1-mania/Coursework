...
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time >= 19 <- !gohome(Me); !sleep(Me, Time).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(TimeNow) & pos(Place,X,Y) & lecture(Name, Week, Day, Time, Place,Priority)  & Time == TimeNow & (Week ==0 | Week==WeekNow)<- add_goal("Going to the next lecture");!goto(Me,X,Y); .concat("Attending ", Name, " lecture",G);add_goal(G); one_hour(Time); .print(Place); !checkevents(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 12 & not (lecture(Name,Week,Day, Time, Place,Priority) & Week == 0 | Week == WeekNow)<- !gotobar(Me); add_goal("Having lunch"); !checkevents(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 17 & Day == 4 & not tired(Me) <- +tired(Me); !givelectures(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 17 & Day == 5 & not tired(Me) <- +tired(Me); !givelectures(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 18 & Day == 1 & not tired(Me) <- +tired(Me); !givelectures(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 18 & Day == 2 & not tired(Me) <- +tired(Me); !givelectures(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 18 & Day == 3 & not tired(Me) <- +tired(Me); !givelectures(Me).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & Time == 17 & tired(Me) <- .print("Tired"); !gohome(Me); !sleep(Me, Time).
+!givelectures(Me) : week(WeekNow) & day(Day) & time(Time) & not (lecture(Name,Week,Day, Time, Place,Priority) & Week == 0 | Week == WeekNow) & Time > 9 & Time <= 19<- !gotoforum(Me); add_goal("Working in forum.");!checkevents(Me).
...
+!gotobar(Me): pos(b1,X,Y) <- add_goal("Going to restaurant!"); !goto(Me,X,Y).
...
+!sleep(Me, Time) : Time < 21 <- add_goal("Sleeping."); !checkevents(Me). 
+!sleep(Me, Time) <- add_goal("Sleeping."); -tired(Me); !checkevents(Me).
